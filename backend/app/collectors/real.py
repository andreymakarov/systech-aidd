"""Real stat collector that reads from the bot database."""
...
class RealStatCollector(StatCollector):
    ...
    def get_stats(self, period: str) -> DashboardStats:
        ...
        active_user_ids = {m.user_id for m in messages if start <= m.created_at <= now}

        total_dialogs = len(sessions_in_period)
        avg_dialog_length = round(
            (sum(s.num_messages for s in sessions_in_period) / total_dialogs)
            if total_dialogs
            else 0.0,
            1,
        )
        ...
        for m in (m for m in messages if start <= m.created_at <= now):
            ...
            if (
                m.user_id not in last_active_per_user
                or m.created_at > last_active_per_user[m.user_id]
            ):
